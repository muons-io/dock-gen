...
public sealed record ContainerPortExtractRequest(Dictionary<string, string> Properties) : IExtractRequest<List<ContainerPort>>
{
    public sealed class ContainerPortExtractRequestHandler : IExtractRequestHandler<ContainerPortExtractRequest, List<ContainerPort>>
    {
        ...
        {
            if (!request.Properties.TryGetValue(MSBuildProperties.ContainerProperties.ContainerPort, out var ports) || !ports.Any())
            {
